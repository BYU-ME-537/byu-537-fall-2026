...
class SerialArm:
    ...
    def __init__(
        self, dh, joint_type=None, base=sp.eye(4), tip=sp.eye(4), radians=True
    ):
        """
        ...
        for i in range(4):
            for j in range(4):
                if isinstance(self.base[i, j], sp.Symbol) or isinstance(
                    self.tip[i, j], sp.Symbol
                ):
                    self.is_symbolic = True
        ...
        for i in range(self.n):
            symbolic_tf = dh2A(
                dh[i], self.joint_type[i], q=sp.symbols("q" + str(i + 1))
            )
    ...
    def jacob_shift(self, q, R, p, index=None):
        ...
        Z = sp.Matrix([[sp.eye(3), -S], [sp.zeros(3), sp.eye(3)]]) @ sp.Matrix(
            [[R, sp.zeros(3)], [sp.zeros(3), R]]
        )
