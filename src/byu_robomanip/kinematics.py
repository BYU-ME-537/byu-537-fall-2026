...
from byu_robomanip.utility import skew
...
def dh2A(dh: list[float], jt: str) -> Callable[[float], NDArray]:
    ...
    if jt == "r":
        ...
        def A(q: float) -> NDArray:
            # See eq. (2.52), pg. 64
            # TODO - complete code that defines the "A" or "T" homogeneous matrix for a given set of DH parameters.
...
class SerialArm:
    ...
    def __init__(
        self,
        dh: list[list[float]],
        jt: list[str] | None = None,
        base: NDArray = np.eye(4),
        tip: NDArray = np.eye(4),
        ...
        if jt is None:
            ...
            assert (
                len(jt) == self.n
            ), "Joint type list does not have the same size as dh param list!"
            self.jt = jt
        ...
        for i in range(self.n):
            ...
            # build one DH transform function for each joint, then append that function to the "transforms"
            # list.
    ...
    def __str__(self):
        ...
        for dh, jt in zip(self.dh, self.jt):
            row = [
                f"{val:.3f}".rstrip("0") if isinstance(val, float) else f"{val}"
                for val in [*dh, jt]
            ]
            dh_string += (
                f"|{''.join([f'{str(s).center(chars_per_col)}|' for s in row])}\n"
            )
        dh_string += f'└{"┴".join(["—"*chars_per_col for i in range(cols)])}┘\n'
        return dh_string
    ...
    def jacob(
        ...
        # TODO: Compute the geometric Jacobian for the selected frame location, assuming
    ...
    def ik_position(
        self,
        target: NDArray,
        q0: list[float] | NDArray | None = None,
        method: str = "J_T",
        force: bool = True,
        tol: float = 1e-4,
        K: NDArray = None,
        ...
        assert isinstance(
            K, np.ndarray
        ), "Gain matrix 'K' must be provided as a numpy array"
        ###############################################################################################
        ###############################################################################################

        # TODO: Implement position inverse kinematics using the requested Jacobian methods.
        ...
        raise NotImplementedError("Complete SerialArm.ik_position for HW06")
    ...
    def Z_shift(
        ...
        """
        # TODO: Implement the wrench/twist shift matrix for the selected frames.
        #       You can use the "skew" function defined in utility.py to generate
        #       the skew-symmetric matrix for the translation vector p.
        #
        #       You should pay attention to what frame the translation vector p is
        #       expressed in as it changes the form of the shifting matrix.
        # HW07 student task

        raise NotImplementedError("Complete SerialArm.Z_shift for HW07")
