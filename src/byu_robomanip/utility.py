    ...
    if isinstance(A, (np.ndarray)):
        if A.ndim == 2:
            with np.printoptions(
                precision=p, suppress=True, floatmode="fixed", linewidth=200
            ):
...
def vector_cubic_interpolation(t0, tf, v0, vf, ms=None, mf=None, deriv=False):
    ...
    for i in range(n):
        if deriv:
            f, fd, fdd = cubic_interpolation(
                t0, tf, v0[i], vf[i], ms[i], mf[i], deriv=True
            )
            funcs.append(f)
            dfuncs.append(fd)
            ddfuncs.append(fdd)
...
def vector_quartic_interpolation(t0, tf, x0, xf, v0, vf, a0, deriv=False):
    ...
    for i in range(n):
        f, fd, fdd = quartic_interpolation(
            t0, tf, x0[i], xf[i], v0[i], vf[i], a0[i], deriv=True
        )
        funcs.append(f)
        dfuncs.append(fd)
        ddfuncs.append(fdd)
...
def quintic_interpolation(
    t0, tf, x0, xf, v0=None, vf=None, a0=None, af=None, deriv=False, cost="minjerk"
):
...
def vector_quintic_interpolation(
    t0, tf, x0, xf, v0=None, vf=None, a0=None, af=None, deriv=False, cost="minjerk"
):
    funcs = []
    dfuncs = []
    ...
    for i in range(n):
        f, fd, fdd = quintic_interpolation(
            t0, tf, x0[i], xf[i], v0[i], vf[i], a0[i], deriv=True, cost=cost
        )
        funcs.append(f)
        dfuncs.append(fd)
        ddfuncs.append(fdd)
    ...
    for i in range(1, m):
        if qds is None:
            ...
            if i == m - 1:
                ...
                for j in range(n):
                    if (qs[i - 1, j] > qs[i, j] < qs[i + 1, j]) or (
                        qs[i - 1, j] < qs[i, j] > qs[i + 1, j]
                    ):
                        mf[j] = 0.0
                    elif i == 1:
        ...
        if oneD:
            f, fd, fdd = cubic_interpolation(
                ts[i - 1], ts[i], qs[i - 1], qs[i], ms, mf, deriv=True
            )
        else:
            f, fd, fdd = vector_cubic_interpolation(
                ts[i - 1], ts[i], qs[i - 1], qs[i], ms, mf, deriv=True
            )
        funcs.append(f)
        dfuncs.append(fd)
...
def quartic_spline(ts, qs, qds, qdd0, deriv=False):
    ...
    for i in range(1, m):
        f, fd, fdd = vector_quartic_interpolation(
            ts[i - 1], ts[i], qs[i - 1], qs[i], qds[i - 1], qds[i], qdd, True
        )
        qdd = fdd(ts[i])
        funcs.append(f)
        dfuncs.append(fd)
...
def quintic_spline(ts, qs, deriv=False, cost="minjerk"):
    ...
    for i in range(1, m):
        f, fd, fdd = vector_quintic_interpolation(
            ts[i - 1],
            ts[i],
            qs[i - 1],
            qs[i],
